...
data OllamaConfig = OllamaConfig
  ...
  , timeout :: Int
  -- ^ Timeout in seconds; will not be honoured when commonManager is provided
  , onModelStart :: Maybe (IO ())
  -- ^ Called when model starts (model name)
  , onModelError :: Maybe (IO ())
  -- ^ Called on error (error message)
  , onModelFinish :: Maybe (IO ())
  -- ^ Called on completion (start, end), will not be executed at error
  , retryCount :: Maybe Int
  -- ^ Retry if any error happened. Default 0
  , retryDelay :: Maybe Int
  -- ^ How many seconds later retry should happen
